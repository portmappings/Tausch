...
public class NoTradeHistoryButton extends Button {
    ...
    private static final ConfigCursor CURSOR = new ConfigCursor(Tausch.getInstance().getMenusConfig(), "no-trade-history-button");

    @Override
    public ItemStack getButtonItem(Player player) {
        String matName = CURSOR.getString("material");
        Material material = matName == null ? Material.BARRIER : Material.valueOf(matName.toUpperCase());
        String display = CC.t(CURSOR.getString("display-name") == null ? "&c&lNo Trade History" : CURSOR.getString("display-name"));
